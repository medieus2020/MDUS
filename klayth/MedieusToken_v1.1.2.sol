...
interface IERC20 {
    ...
    event BurnFrom(address indexed account, uint256 value);
}
/**
...
contract Lockable {
    ...
    modifier CheckLockFormAddress(address sender2) {
        if (m_mLockAddress[sender2]) {
            revert();
        }
        _;
    }
...
contract ERC20 is Context, IERC20, Lockable {
   ...
   CheckLockFormAddress(_msgSender())
    public returns (bool) {
...
contract MedieusToken is ERC20Pausable, ERC20Burnable,  MultiTransferToken  {
  string public constant name = "Medieus Token";
  string public constant symbol = "MDUS";
